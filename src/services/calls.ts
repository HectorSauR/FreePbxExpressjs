...
class CallsService {
    ...
    static getAllMatriz = async () => {     
        let [rows] = await connectionMatriz.execute('SELECT * FROM cdr');
    ...
    static getAllSuc1 = async () => {
        let rows: any;
        try {
            [rows] = await connectionSuc1_replication.execute('SELECT * FROM cdr');
        }catch(e: any){
            [rows] = await connectionSuc1.execute('SELECT * FROM cdr');
        }
    ...
    static getAllSuc2 = async () => {     
        let rows: any;
        try {
            [rows] = await connectionSuc2_replication.execute('SELECT * FROM cdr');
        }catch(e: any){
            [rows] = await connectionSuc2.execute('SELECT * FROM cdr');
        }
